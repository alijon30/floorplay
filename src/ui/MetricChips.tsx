...
function Chip({ label, value, tone, hint }: { label: string; value: string; tone: 'ok' | 'warn' | 'bad'; hint: string }) {
  ...
  return <div title={hint} className={`flex h-7 shrink-0 items-center rounded-full border px-3 text-xs ${color}`}><span className="text-neutral-500">{label}&nbsp;</span>{value}</div>;
}
