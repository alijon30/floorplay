...
export const BTN = `${BASE} border border-line bg-raised text-fg hover:border-[var(--line-hi)] hover:bg-[var(--raised-hi)]`;

/** The same button while its panel is open or its mode is on. */
export const BTN_ON = `${BASE} border border-accent/60 bg-[var(--accent-fill)] text-accent`;

/** The action a card is really for: Apply, Accept, Place, Create. */
export const BTN_PRIMARY = `${BASE} bg-accent font-medium text-[var(--accent-ink)] hover:bg-[var(--accent-hi)] disabled:bg-raised disabled:text-muted disabled:opacity-100`;

/** A quieter action beside it: Reject, Lock, Select, Cancel. */
export const BTN_QUIET = `${BASE} border border-transparent bg-raised text-fg hover:bg-[var(--raised-hi)]`;
...
export const BTN_SM = `${SMALL} border border-line bg-raised text-muted hover:text-fg hover:border-[var(--line-hi)]`;
export const BTN_SM_ON = `${SMALL} border border-accent/60 bg-[var(--accent-fill)] text-accent`;
...
/** The same, lit. */
export const ICON_BTN_ON = `inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-[var(--accent-fill)] text-accent transition-colors ${FOCUS} disabled:opacity-30`;

/** A text field, number field or select. */
export const INPUT = `h-8 w-full min-w-0 rounded-md border border-line bg-raised px-2 text-[12.5px] text-fg outline-none transition-colors placeholder:text-muted hover:border-[var(--line-hi)] focus:border-accent/70 ${FOCUS}`;

/** The link-weight buttons at the foot of a panel. */
export const LINK = `rounded text-[11.5px] text-accent transition-colors hover:text-[var(--accent-hi)] ${FOCUS}`;
...
export const SEG_ITEM_ON = `inline-flex h-[22px] shrink-0 items-center justify-center rounded-[4px] bg-[var(--accent-fill)] px-2 text-[11px] font-medium text-accent transition-colors ${FOCUS}`;
