...
export default function StylePopover({ onClose, anchorRef }: { onClose: () => void; anchorRef: RefObject<HTMLElement | null> }) {
  ...
  return (
    <div role="dialog" aria-label="Style" className="absolute right-0 z-30 mt-1 max-h-[70vh] w-72 overflow-auto rounded border border-neutral-700 bg-neutral-900 p-3 text-xs shadow-xl">
      ...
      <strong className="block text-[11px] uppercase tracking-wide text-neutral-500">Walls</strong>
      <div className="mt-1 grid grid-cols-4 gap-2">
        {WALL_SWATCHES.map((s) => (
          <button
            ...
            className={`h-8 w-full rounded ring-offset-2 ring-offset-neutral-900 ${room.finish.wall === s.hex ? 'ring-2 ring-emerald-400' : 'ring-1 ring-neutral-700 hover:ring-neutral-400'}`}
            style={{ background: s.hex }}
          />
        ))}
      </div>

      <strong className="mt-3 block text-[11px] uppercase tracking-wide text-neutral-500">Floor</strong>
      <div className="mt-1 flex flex-wrap gap-1.5">
        {FLOOR_FINISHES.map((f) => (
          <button
            ...
            onClick={() => setFloor(f)}
            className={`flex items-center gap-1.5 rounded border px-2 py-1 ${room.finish.floor === f ? 'border-emerald-500 text-emerald-300' : 'border-neutral-700 text-neutral-300 hover:border-neutral-500'}`}
          >
      ...
      <strong className="mt-3 block text-[11px] uppercase tracking-wide text-neutral-500">Suggest palettes</strong>
      <p className="mt-1 text-neutral-500">Read from what is already in the room.</p>
      <div className="mt-1 space-y-1.5">
        {palettes.map((p) => (
          <div key={p.name} className="rounded border border-neutral-800 p-2">
            <div className="flex items-center justify-between gap-2">
              ...
              <button className="rounded bg-emerald-700 px-2 py-0.5 text-[11px] text-white hover:bg-emerald-600" onClick={() => applyPalette(p)}>Apply</button>
            </div>
