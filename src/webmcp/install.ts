...
import { buildHomeTools } from './tools/homeTools';
import { withConsent } from './tools/consent';
...
export function installWebMCP(store: RoomStore, mcOverride?: ModelContextLike): { registry: ToolRegistry; isNative: boolean } {
  ...
  const staticTools = [...buildReadTools(ctx), ...buildMutateTools(ctx), ...buildWallTools(ctx), ...buildShoppingTools(ctx), ...buildHomeTools(ctx), ...buildProposalTools(ctx)].map((t) => withConsent(ctx, t));
